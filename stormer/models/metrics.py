...
def raw_mse(pred, y, vars):
    ...
    for i, var in enumerate(vars):
        loss_dict[f"w_mse_{var}"] = (error[:, i]).mean()

    loss_dict["w_mse_aggregate"] = (error).mean()

    return loss_dict


def raw_mse_masked(pred, y, vars, mask):
    """mean squared error
...
def raw_mse_masked_nan(pred, y, vars, mask):
    """mean squared error
    ...
    error[mask < 0.00001] = float("nan")

    loss_dict = {}
    for i, var in enumerate(vars):
        loss_dict[f"w_mse_{var}"] = torch.nanmean(error[:, i])

    loss_dict["w_mse_aggregate"] = torch.nanmean(error)
...
def lat_weighted_mse(pred, y, vars, lat, weighted=False, weight_dict=None):
    ...
    if weighted:
        weights = (
            torch.Tensor([weight_dict[var] for var in vars])
            .to(device=error.device)
            .view(1, -1, 1, 1)
        )
...
def lat_weighted_mse_val(
    pred, y, transform, vars, lat, log_postfix, weighted=False, weight_dict=None
):
    """Latitude weighted mean squared error
    Args:
        y: [B, V, H, W]
    ...
    loss_dict[f"w_mse_aggregate_{log_postfix}"] = (
        (error * w_lat.unsqueeze(1) * weights).sum(dim=1).mean()
    )
...
def lat_weighted_spread_skill_ratio(
    ...
    rmse_dict = lat_weighted_rmse(
        pred.mean(dim=1), y, transform, vars, lat, log_postfix, weighted, weight_dict
    )
    ...
    loss_dict = {}
    for i, var in enumerate(vars):
        loss_dict[f"w_ssr_{var}_{log_postfix}"] = (
            spread[i] / rmse_dict[f"w_rmse_{var}_{log_postfix}"]
        )
...
def lat_weighted_acc(pred, y, transform, vars, lat, clim, log_postfix):
    ...
    w_lat = w_lat / w_lat.mean()  # (H, )
    w_lat = (
        torch.from_numpy(w_lat).unsqueeze(0).unsqueeze(-1).to(dtype=pred.dtype, device=pred.device)
    )  # [1, H, 1]
    ...
    with torch.no_grad():
        for i, var in enumerate(vars):
            ...
            ) / torch.sqrt(torch.sum(w_lat * pred_prime**2) * torch.sum(w_lat * y_prime**2))
