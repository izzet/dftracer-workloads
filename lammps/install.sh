#!/usr/bin/env bash
# install.sh — one-time environment setup for the LAMMPS workload.
# Idempotent: safe to re-run.  Performs:
#   1. git submodule add/sync/update for LAMMPS source (pinned to stable tag)
#   2. spack -e . concretize + install (compiles LAMMPS — ~10-20 min)
...
SUBMODULE_URL="https://github.com/lammps/lammps.git"
SUBMODULE_TAG="stable_29Aug2024_update4"
SUBMODULE_REL_PATH="${WORKLOAD_NAME}/lammps"
...
# ---------------------------------------------------------------------------
# 1. Git submodule (source reference)
...
log_msg "Resetting LAMMPS submodule to clean state (patches reapplied below) ..."
...
# ---------------------------------------------------------------------------
# Apply patches from patches/ (if any)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Spack environment: concretize + install LAMMPS + all deps
#    spack.yaml has a `develop:` block pointing at ./lammps (the submodule).
#    Spack will compile LAMMPS FROM the local source, not a downloaded tarball.
#    This allows source edits + incremental rebuilds via ./build.sh.
#    NOTE: first run compiles LAMMPS from scratch (~10-20 min).
# ---------------------------------------------------------------------------
log_msg "spack concretize ..."
spack -e "${ROOT_DIR}" concretize -f >> "${SPACK_LOG}" 2>&1
log_msg "spack concretize done"
log_msg "spack install — builds LAMMPS from ./lammps source (first run: ~10-20 min) ..."
